...
def reverse_polish_notation():
    """
    Обратная польская запись или нотация
    Вычисляет арифметические выражений в постфиксной нотации
    Можно написать арифметические выражения что скобки не нужны
    [5, 2, "+"] <=> 5 + 2
    (2+7)*5 <=> [2, 7, +, 5, *]
    2+7*5 <=> [2, 7, 5, *, +]
    :return:
